import {
  Meta,
  Story,
  Props,
  Preview,
  Canvas,
} from "@storybook/addon-docs/blocks";
import {
  withKnobs,
  select,
  boolean,
  text,
  number,
} from "@storybook/addon-knobs";
import { Button, colors } from "demo-blend-ui/core";
import { BlendIcon } from "@blend-ui/icons";

import { buttonColorArgTypes } from "./shared/themeProps";

import eyeIcon from "@iconify/icons-bx/bx-show";

<Meta
  title="Core/Button"
  component={Button}
  parameters={{
    options: { showPanel: true },
    decorators: [withKnobs],
  }}
/>

# Button

Experiment with this story with Controls addon in Canvas mode.

<Preview withToolbar>
  <Story name="Button Interactive" argTypes={buttonColorArgTypes}>
    {(args) => <Button {...args}>Default</Button>}
  </Story>
</Preview>

<Props of={Button} />

<Preview withToolbar>
  <Story name="Outline">
    {(args) => (
      <Button {...args} variation={"outline"}>
        Outline
      </Button>
    )}
  </Story>
</Preview>

<Preview withToolbar>
  <Story name="Disabled">
    <Button disabled>Disabled</Button>
  </Story>
</Preview>

<Preview withToolbar>
  <Story name="Button Link">
    <Button variation={"link"}>Something</Button>
  </Story>
</Preview>

<Preview withToolbar>
  <Story name="Disabled Button Link">
    <Button disabled variation={"link"}>
      Something
    </Button>
  </Story>
</Preview>

### Variations

<Canvas withToolbar>
  <Story name="Size Variations">
    <div
      style={{
        height: 100,
        width: 600,
        flexDirection: "row",
        justifyContent: "space-between",
      }}
    >
      <Button size="lg">Large</Button>
      <Button size="md">Medium</Button>
      <Button size="sm">Small</Button>
      <Button size="xs">Extra small</Button>
    </div>
  </Story>
</Canvas>

<Canvas withToolbar>
  <Story name="State Variations">
    <div
      style={{
        height: 100,
        width: 600,
        flexDirection: "row",
        justifyContent: "space-between",
      }}
    >
      <Button>Default</Button>
      <Button
        style={{ backgroundColor: colors.baseHover, border: colors.baseHover }}
      >
        Hover
      </Button>
      <Button disabled>Disabled</Button>
    </div>
  </Story>
</Canvas>

<Canvas withToolbar>
  <Story name="Style Variations">
    <div
      style={{
        height: 100,
        width: 60,
        flexDirection: "row",
        justifyContent: "space-between",
      }}
    >
      <Button variation={"fill"}>Fill</Button>
      <Button variation={"outline"}>Outline</Button>
      <Button variation={"link"}>Link</Button>
      <Button variation={"file"}>File</Button>
    </div>
  </Story>
</Canvas>

<Canvas withToolbar>
  <Story name="Color Variations">
    <div>
      <Button variation={"fill"} colorStyle={"secondary"}>
        Default
      </Button>
      <Button variation={"fill"} colorStyle={"error"}>
        Default
      </Button>
      <Button variation={"outline"} colorStyle={"secondary"}>
        Outline
      </Button>
      <Button variation={"outline"} colorStyle={"error"}>
        Outline
      </Button>
    </div>
  </Story>
</Canvas>

<!-- <Canvas withToolbar>
  <Story name="Icon Button">
    <Button size="xs">
      <BlendIcon iconify={eyeIcon} size={"16"} />
    </Button>
  </Story>
</Canvas>

<Canvas withToolbar>
  <Story name="Icon Left">
    <Button
      onChange={(e) => {
        console.log(e);
      }}
    >
      Icon Right
      <BlendIcon iconify={eyeIcon} size="12px" />
    </Button>
  </Story>
</Canvas>

<Canvas withToolbar>
  <Story name="Icon Right">
    <Button
      onChange={(e) => {
        console.log(e);
      }}
    >
      <BlendIcon iconify={eyeIcon} size="12px" />
      Icon Left
    </Button>
  </Story>
</Canvas>

<Canvas withToolbar>
  <Story name="Icon Right and Left">
    <Button
      onChange={(e) => {
        console.log(e);
      }}
    >
      <BlendIcon iconify={eyeIcon} size="12px" />
      Left & Right
      <BlendIcon iconify={eyeIcon} size="12px" />
    </Button>
  </Story>
</Canvas> -->

### Playground
